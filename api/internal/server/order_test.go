...
func submit(ctx *gin.Context) {
	...
	m["time"] = int(m["time"].(float64))
	fmt.Println(m)

	fromsign := m["sign"]
	key := "67twt1POakMStQWI5euIpq9RCkjWeFAJ"
	selfsign := createsign(m, key)

	fmt.Println(fromsign)
	fmt.Println(selfsign)
